...
完全に自己完結したアプリにするには、binary（Node.js など）を Tauri sidecar として `externalBin` でバンドルする。これによりシステム依存を完全に排除できる。詳細は [sidecar パターン](../architecture/sidecar-pattern.mdx)のページを参照のこと。
...
ローディング用 HTML ページを含む完全なパターンについては、[ローディング画面](../architecture/loading-screen.mdx)ページを参照のこと。
