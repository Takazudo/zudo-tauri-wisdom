...
[アセットプロトコルと CSP スコープ](../frontend/asset-protocol.mdx)ページでは、`convertFileSrc` の URL を実際に読み込み可能にする `tauri.conf.json` の設定を解説している。このレシピで使われる `csp: null` ショートカットと、ハードニング済みビルドで必要になる変更内容も含む。
