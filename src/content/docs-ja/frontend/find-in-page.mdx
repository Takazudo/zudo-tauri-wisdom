...
`NodeFilter.SHOW_TEXT` はコンテナ内の `<script>` や `<style>` タグのテキストノードも収集する。`<mark>` 要素がスクリプトやスタイルのテキストをラップしてしまうのを避けるため、コンテナセレクターは `document.body` ではなくコンテンツ要素（例：`article.main-content`）にスコープする。
...
`__TAURI_INTERNALS__` は Tauri の内部実装の詳細である。Tauri v2 のリリース間で事実上安定しているが、パブリック API の一部ではない。より公式なチェックが必要な場合は、最近の Tauri v2 リリースの `@tauri-apps/api/core` パッケージが公式のパブリック代替として `isTauri()` を提供している。
...
- **find-in-page インスタンスに `useRef`** -- ユーティリティは内部状態（マッチリスト、アクティブインデックス）を保持する。`useRef` を使用することで、不要な再レンダリングを引き起こさずに 1 つのインスタンスが再レンダリング間で永続化される。なお `useRef(createFindInPage())` はレンダリングごとにファクトリ関数を呼び出す（React は初回値のみ使用）。完全なレイジーイニシャライズが必要な場合は `useRef<FindInPage | null>(null)` で初期化し、`useEffect` 内でインスタンスを作成する。
...
- **デフォルトではデバウンスなし** -- `handleFind` はキーストロークごとに DOM ツリー走査全体を実行する。ほとんどのドキュメントサイトでは十分に高速だが、非常に大きなページ（数千の DOM ノード）では `findInPage.find()` の呼び出しにデバウンス（例：100〜150ms）を追加することを検討する。
...
- [ウィンドウの作成とライフサイクル](../rust-backend/window-management.mdx) -- OS ブラウザで外部リンクを開くための `on_navigation` について説明している。同様にカスタム対応が必要な WebView の動作である
