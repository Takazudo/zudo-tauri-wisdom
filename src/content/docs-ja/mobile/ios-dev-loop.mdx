...
本番で独自のバックエンド URL（例: HTTP で配信される REST API）にアクセスする場合は、[`Info.ios.plist`](./ios-project-structure.mdx#info-ios-plist-%E8%BF%BD%E5%8A%A0%E3%82%AD%E3%83%BC%E3%81%AE%E3%83%9E%E3%83%BC%E3%82%B8) に `NSExceptionDomains` エントリを追加すること。広範な `NSAllowsArbitraryLoads` は有効にしないこと -- App Store 審査の Guideline 4.2 で問題になる。
