...
6. **`on_navigation` でリンクの動作を制御する** -- 外部リンクはデフォルトブラウザで開く。フロントエンドからのプログラマティックな URL 開封には `tauri-plugin-opener` を使用する
...
- [Find-in-Page](../frontend/find-in-page.mdx) -- カスタム実装が必要な別の WebView の動作：Tauri はブラウザのネイティブな Ctrl+F 検索ダイアログを公開していない
