...
A Tauri webview behaves like an embedded browser -- when a user clicks an `<a href="https://example.com">` link, the webview navigates to that URL in-place, replacing your app's UI. This is the natural default, but in most apps you want external links to open in the OS default browser instead.

There are two complementary approaches:

1. **Rust-side `on_navigation` callback** -- intercepts all top-level navigations before they happen. Best as a catch-all safety net.
2. **Frontend-side `tauri-plugin-opener`** -- lets you programmatically open URLs from JavaScript. Best for explicit "open in browser" actions.

### Rust: `on_navigation` callback

The `on_navigation` callback controls which URLs the webview is allowed to load. Return `true` to allow the navigation, `false` to block it. For external URLs, open them in the default browser before returning `false`:
...
This uses the `open` crate (`open::that()`) to launch the OS default browser. Add it to your `Cargo.toml`:
...
`on_navigation` only fires for top-level navigations (clicking links, `window.location` changes). It does not intercept `fetch()` requests, `XMLHttpRequest`, or iframe navigations. For those, use Tauri's security scope and CSP settings.

</Note>

### Frontend: `tauri-plugin-opener`

For cases where you want to open a URL from JavaScript -- for example, a button click handler or a dynamically generated link -- use the official `tauri-plugin-opener`:
...
Add the permission in your capability file:
...
Use both approaches together: `on_navigation` as the Rust-side safety net that catches all link clicks, and `tauri-plugin-opener` for explicit frontend actions like "Open in browser" buttons. The `on_navigation` callback ensures no external URL ever accidentally loads inside the webview, even if you miss a link somewhere in your frontend code.
...
6. **Use `on_navigation` to control link behavior** -- open external links in the default browser; use `tauri-plugin-opener` for programmatic URL opening from the frontend
...
- [Find-in-Page](../frontend/find-in-page.mdx) -- another webview behavior that needs custom implementation: Tauri does not expose the browser's native Ctrl+F find dialog
