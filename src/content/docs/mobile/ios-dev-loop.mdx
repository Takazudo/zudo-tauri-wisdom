...
If you use a custom backend URL at runtime (e.g. a REST API served over HTTP in production), you have to add an `NSExceptionDomains` entry in [`Info.ios.plist`](./ios-project-structure.mdx#info-ios-plist-merging-extra-keys). Don't set the broad `NSAllowsArbitraryLoads` -- it will cause App Store review issues under Guideline 4.2.
