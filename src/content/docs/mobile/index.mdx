...
- [iOS Input Auto-Zoom on Focus](./ios-input-auto-zoom.mdx) -- why `WKWebView` zooms into inputs under 16px and how to prevent it without disabling pinch-zoom
- [iOS Native Plugins](./native-plugins.mdx) -- loading `tauri-plugin-haptics` and `tauri-plugin-notification` via guarded dynamic import so the same frontend bundle runs on web, desktop, and iOS without crashing
