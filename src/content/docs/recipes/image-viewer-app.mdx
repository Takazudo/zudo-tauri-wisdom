...
The [Asset Protocol and CSP Scope](../frontend/asset-protocol.mdx) page explains the `tauri.conf.json` config that makes `convertFileSrc` URLs actually load -- including the `csp: null` shortcut used by this recipe and what you need to change for a hardened build.
