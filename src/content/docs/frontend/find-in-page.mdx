...
`NodeFilter.SHOW_TEXT` collects text nodes from all elements, including `<script>` and `<style>` tags if they are inside the container. Scope your container selector to a content element (e.g., `article.main-content`) rather than `document.body` to avoid accidentally wrapping script/style text in `<mark>` elements.
...
`__TAURI_INTERNALS__` is an internal Tauri implementation detail. It has been stable in practice across Tauri v2 releases, but it is not part of the public API. If you need a more official check, Tauri's `@tauri-apps/api/core` package exposes `isTauri()` in recent Tauri v2 releases as a proper public alternative.
...
- **`useRef` for the find-in-page instance** -- the utility maintains internal state (match list, active index). Using `useRef` ensures one instance persists across re-renders without triggering unnecessary updates. Note that `useRef(createFindInPage())` calls the factory on every render (React only uses the initial value); for a clean lazy-init, use `useRef<FindInPage | null>(null)` and initialize inside a `useEffect`.
...
- **No debounce by default** -- `handleFind` runs the full DOM tree walk on every keystroke. For most doc sites this is fast enough, but on very large pages (thousands of DOM nodes) you may want to add a debounce (e.g., 100–150ms) around the `findInPage.find()` call.
...
- [Window Creation and Lifecycle](../rust-backend/window-management.mdx) -- covers `on_navigation` for opening external links in the OS browser, a related webview behavior that also needs custom handling
