...
## Frontend plugin calls: clipboard

Not every frontend-to-backend call is a custom `#[tauri::command]`. Tauri's official plugins expose ready-made functions you `import` directly from the JS SDK. Writing to the clipboard is the canonical example: a "copy export to clipboard" button uses `writeText` from `@tauri-apps/plugin-clipboard-manager`.
...
### Why the plugin instead of `navigator.clipboard`

The browser's `navigator.clipboard.writeText` exists in the WebView, but it is unreliable there. `navigator.clipboard` is gated by the secure-context requirement and by browser-style permission prompts, neither of which behaves consistently across the macOS, Windows, and Linux WebView implementations Tauri ships on. Calls can silently reject or hang depending on the platform and how the window was focused.

The plugin routes the write through the native Rust clipboard API instead of the WebView's JS clipboard. That sidesteps the secure-context and permission quirks entirely, so the same call works the same way on every platform.

### Capability and plugin init

Two pieces of setup are required, and a missing one fails at runtime, not compile time.

First, grant the `clipboard-manager:default` permission in the window's capability file:
...
Second, register the plugin in `main.rs` (and add the `tauri-plugin-clipboard-manager` crate plus the `@tauri-apps/plugin-clipboard-manager` npm package):
...
If the capability is missing, `writeText` rejects with a permission error at runtime. If the plugin is not initialized, the call fails because the underlying command does not exist. Neither shows up at compile time -- the failure only surfaces when the button is clicked.
